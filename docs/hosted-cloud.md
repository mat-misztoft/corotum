...
ToolMirror's hosted Worker sends transactional sign-in email through the Cloudflare Email Service `send_email` binding. It does **not** need a separate email API key.
...
2. Onboard `toolmirror.com` as a sending domain in Cloudflare Email Service and publish every required sending-domain DNS/authentication record (including the records Cloudflare supplies for sender authentication).
3. Configure the ToolMirror authentication sender as `auth@toolmirror.com`; it must be an allowed sender address on that onboarded domain.
4. Keep the Worker `send_email` binding named `EMAIL` with `auth@toolmirror.com` in its allowed sender addresses. `apps/web/wrangler.jsonc` is the hosted binding configuration.
...
Default origin is `https://toolmirror.com`. After checkout and a verified webhook, Cloud init can write desired state. Device pairing and authentication without a subscription do not grant Cloud Sync.
