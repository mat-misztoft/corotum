...
test("dashboard registers the planned WebMCP tools through document.modelContext", () => {
  for (const tool of ["list_skills", "list_devices", "get_sync_status", "check_skill_updates"])
    expect(source).toContain(`"${tool}"`);
  for (const tool of ["add_skill", "remove_skill", "update_skill", "set_skill_ref"])
...
test("landing exposes only a dashboard navigation tool", () => {
  ...
  expect(source).toContain('landing ? [openDashboardTool] : dashboardTools');
});
