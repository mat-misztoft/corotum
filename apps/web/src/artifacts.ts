...
export type ArtifactBucket = {
  ...
  list(prefix: string): Promise<{ keys: readonly string[]; truncated: boolean }>;
...
export class ArtifactTransferError extends Error {
  ...
  constructor(
    readonly code: "ARTIFACT_UNAVAILABLE" | "CONTENT_HASH_MISMATCH" | "VALIDATION_ERROR",
    message: string,
  ) {
    super(message);
  }
...
export async function verifyArtifactArchive(
  bytes: Uint8Array,
  expected: Pick<ArtifactDescriptor, "contentHash" | "integrityHash" | "sizeBytes">,
): Promise<void> {
  if (bytes.byteLength !== expected.sizeBytes) {
    throw new ArtifactTransferError("CONTENT_HASH_MISMATCH", "Artifact size does not match.");
  }
  if ((await sha256(bytes)) !== expected.integrityHash) {
    throw new ArtifactTransferError("CONTENT_HASH_MISMATCH", "Artifact integrity hash does not match.");
  }
  const tar = await decompressZstd(bytes);
  const files = parseTar(tar);
  if ((await contentHash(files)) !== expected.contentHash) {
    throw new ArtifactTransferError("CONTENT_HASH_MISMATCH", "Extracted artifact content hash does not match.");
  }
}
...
export async function putWorkspaceArtifact(
  ...
  if (transfer.artifact.locator !== locator) {
    throw new ArtifactTransferError("VALIDATION_ERROR", "Artifact locator is not valid for this workspace.");
  }
  ...
  return { skillId: transfer.skillId, artifact: { ...transfer.artifact, locator } };
}
...
export async function getWorkspaceArtifact(
  ...
  if (!bytes) {
    throw new ArtifactTransferError("ARTIFACT_UNAVAILABLE", "Artifact object is missing.");
  }
...
export async function collectArtifactGcPlan(
  ...
  const workspace = await db
    .prepare("SELECT current_revision_sequence AS sequence FROM workspaces WHERE id = ?")
    .bind(workspaceId)
    .first<{ sequence: number }>();
  if (!workspace) throw new ArtifactGcAmbiguousError("Workspace revision pointer is missing.");
  ...
  if (listed.truncated) throw new ArtifactGcAmbiguousError("Artifact object listing is incomplete.");
  ...
  for (const row of referenceRows) {
    ...
    referenced.add(id);
    if (row.sequence === workspace.sequence || row.sequence === previous) retained.add(id);
  }

  const metadataById = new Map<string, (typeof metadataRows)[number]>();
  for (const row of metadataRows) {
    const expected = cloudArtifactLocator(workspaceId, row.skillId, row.integrityHash);
    if (row.locator !== expected) throw new ArtifactGcAmbiguousError("Artifact metadata locator does not match.");
  ...
  for (const key of listed.keys) {
    if (!parseWorkspaceArtifactKey(workspaceId, key)) {
      throw new ArtifactGcAmbiguousError("R2 contains a key outside the artifact contract.");
    }
  }
  for (const id of retained) {
    const row = metadataById.get(id);
    if (!row || !r2Keys.has(row.locator)) {
      throw new ArtifactGcAmbiguousError("A retained artifact object or metadata row is missing.");
    }
  }
  ...
  for (const id of referenced) {
    ...
    const row = metadataById.get(id);
    if (!row || !r2Keys.has(row.locator) || !HASH.test(row.contentHash) || !HASH.test(row.integrityHash)) {
      throw new ArtifactGcAmbiguousError("An unreferenced artifact cannot be deleted safely.");
    }
    candidates.push({
      skillId: row.skillId,
      artifact: {
...
export async function garbageCollectWorkspaceArtifacts(
  ...
  for (const candidate of candidates) {
    ...
    const removed = await db
      ...
      .run();
    const changes = (removed as { meta?: { changes?: number } }).meta?.changes ?? 0;
    if (changes !== 1) throw new ArtifactGcAmbiguousError("Artifact metadata delete did not match the plan.");
  }
  return candidates;
}
...
function parseWorkspaceArtifactKey(workspaceId: string, key: string): ArtifactTransfer | null {
  ...
  if (rest.slice(slash + 1).includes("/") || !HASH.test(integrityHash)) return null;
  try {
    parseSkillId(skill);
  } catch {
    return null;
...
function parseTar(tar: Uint8Array): ReadonlyArray<Readonly<{ path: string; content: Uint8Array }>> {
  if (tar.length < BLOCK * 2 || tar.length % BLOCK !== 0) {
    throw new ArtifactTransferError("ARTIFACT_UNAVAILABLE", "Artifact TAR is malformed.");
  }
  ...
  while (offset < tar.length) {
    ...
    if (header.every((byte) => byte === 0)) {
      if (!tar.slice(offset).every((byte) => byte === 0)) {
        throw new ArtifactTransferError("ARTIFACT_UNAVAILABLE", "Artifact TAR has trailing data.");
      }
      return entries;
    }
    if (offset + BLOCK > tar.length || !validChecksum(header)) {
      throw new ArtifactTransferError("ARTIFACT_UNAVAILABLE", "Artifact TAR header is invalid.");
    }
    const type = String.fromCharCode(header[156]!);
    if (type !== "\0" && type !== "0") {
      throw new ArtifactTransferError("ARTIFACT_UNAVAILABLE", "Artifact TAR contains a non-regular entry.");
    }
    const name = readString(header, 0, 100);
    const prefix = readString(header, 345, 155);
    const path = prefix ? `${prefix}/${name}` : name;
    assertArchivePath(path);
    if (paths.has(path)) throw new ArtifactTransferError("ARTIFACT_UNAVAILABLE", "Artifact TAR contains duplicate paths.");
    paths.add(path);
    const size = readOctal(header, 124, 12);
    if (!Number.isSafeInteger(size) || size < 0 || size > MAX_EXPANDED_BYTES) {
      throw new ArtifactTransferError("ARTIFACT_UNAVAILABLE", "Artifact TAR entry size is invalid.");
    }
    expanded += size;
    if (entries.length >= MAX_ENTRIES || expanded > MAX_EXPANDED_BYTES) {
      throw new ArtifactTransferError("ARTIFACT_UNAVAILABLE", "Artifact exceeds extraction limits.");
    }
    const start = offset + BLOCK;
    const end = start + size;
    if (end > tar.length) throw new ArtifactTransferError("ARTIFACT_UNAVAILABLE", "Artifact TAR is truncated.");
    entries.push({ path, content: tar.slice(start, end) });
    offset = end + ((BLOCK - (size % BLOCK)) % BLOCK);
  }
  throw new ArtifactTransferError("ARTIFACT_UNAVAILABLE", "Artifact TAR has no end marker.");
}
...
function assertArchivePath(path: string): void {
  ...
  if (
    ...
    throw new ArtifactTransferError("ARTIFACT_UNAVAILABLE", "Artifact contains an unsafe path.");
  }
}
...
async function contentHash(
  ...
  for (const file of files) {
    parts.push(encoder.encode(`${file.path}\0`), file.content, encoder.encode("\0"));
  }
