...
export type RevisionActor = Readonly<{ type: "user" | "device" | "system"; id: string }>;
export type CloudDesiredState = DesiredState | V2DesiredState;

export function isV2CloudState(state: CloudDesiredState): state is V2DesiredState {
...
type BoundStatement = ReturnType<ReturnType<WorkspaceDatabase["prepare"]>["bind"]>;
type RevisionDatabase = WorkspaceDatabase & { batch(statements: readonly BoundStatement[]): Promise<readonly RunResult[]> };
...
function normalizeLedger(ledger: DispositionLedger | undefined): DispositionLedger {
  return parseDispositionLedger(serializeDispositionLedger(ledger ?? emptyLedger));
}

function materializedSkillStatements(db: RevisionDatabase, workspaceId: string, revision: CloudRevision, now: number): BoundStatement[] {
  ...
  if (isV2CloudState(state)) {
    return state.manifest.skills.map((skill) => {
      ...
      const contentHash = lock?.materialization.kind === "artifact"
      ...
      ).bind(workspaceId, skill.id, source?.repository ?? null, skill.name, source?.ref ?? null,
        serialize(skill.targets), sourceLock?.repository ?? source?.repository ?? null,
        sourceLock?.revision ?? null, sourceLock?.path ?? source?.path ?? null, contentHash,
        skill.resolutionStatus, now, revision.id, workspaceId);
    });
  }
  return state.manifest.skills.map((skill) => {
    const lock = state.lockfile.skills.find((candidate) => candidate.id === skill.id);
    return db.prepare(
      `INSERT INTO workspace_skills
       ...
       SELECT ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?
       WHERE EXISTS (SELECT 1 FROM workspace_revisions WHERE id = ? AND workspace_id = ?)`,
    ).bind(workspaceId, skill.id, skill.source, skill.skill, skill.ref, serialize(skill.targets),
      lock?.repository ?? null, lock?.revision ?? null, lock?.path ?? null, lock?.contentHash ?? null,
      skill.resolutionStatus, now, revision.id, workspaceId);
  });
}

/** Artifact descriptors are metadata only; R2 locators/bytes never enter workspace_skills. */
function artifactStatements(db: RevisionDatabase, workspaceId: string, revision: CloudRevision, now: number): BoundStatement[] {
  const state = revision.state;
    ...
    return [
      db.prepare(
        `INSERT OR IGNORE INTO workspace_artifacts
         ...
         SELECT ?, ?, ?, ?, ?, ?, ?, ?
         WHERE EXISTS (SELECT 1 FROM workspace_revisions WHERE id = ? AND workspace_id = ?)`,
      ).bind(workspaceId, lock.id, artifact.integrityHash, artifact.kind, artifact.contentHash,
        artifact.locator, artifact.sizeBytes, now, revision.id, workspaceId),
      db.prepare(
        `INSERT INTO workspace_revision_artifacts (revision_id, workspace_id, skill_id, integrity_hash)
         SELECT ?, ?, ?, ?
         WHERE EXISTS (SELECT 1 FROM workspace_revisions WHERE id = ? AND workspace_id = ?)`,
      ).bind(revision.id, workspaceId, lock.id, artifact.integrityHash, revision.id, workspaceId),
    ];
  });
}
 ...
 */
export async function mutateDesiredState(db: RevisionDatabase, input: DesiredStateMutation): Promise<CloudRevision> {
  ...
  ).bind(input.idempotencyKey, input.actor.id).first<{ responseJson: string }>();
  ...
  ).bind(input.workspaceId, input.userId).first<{ currentRevisionSequence: number }>();
  if (!workspace) throw new RevisionConflictError();
  const now = Date.now();
  const revision: CloudRevision = { id: revisionId(), sequence: workspace.currentRevisionSequence + 1, state, dispositionLedger };
  ...
  const statements: BoundStatement[] = [
    db.prepare(`INSERT INTO idempotency_records (key, actor_type, actor_id, operation, response_json, created_at, expires_at)
      SELECT ?, ?, ?, 'desired_state_mutation', ?, ?, ? FROM workspaces w
      WHERE w.id = ? AND w.owner_user_id = ? AND ${baseMatches}`)
      .bind(input.idempotencyKey, input.actor.type, input.actor.id, responseJson, now, now + 86_400_000, input.workspaceId, input.userId, ...baseParameters),
    db.prepare(`INSERT INTO workspace_revisions
      ...
      WHERE w.id = ? AND w.owner_user_id = ? AND ${baseMatches}`)
      .bind(revision.id, serialize(state.manifest), serialize(state.lockfile), serialize(dispositionLedger), now,
        input.actor.type, input.actor.id, input.transition.type, input.transition.skillId,
        serializeRevisionTransition(input.transition), input.workspaceId, input.userId, ...baseParameters),
    db.prepare("DELETE FROM workspace_skills WHERE workspace_id = ? AND EXISTS (SELECT 1 FROM workspace_revisions WHERE id = ? AND workspace_id = ?)")
    ...
    db.prepare("UPDATE workspaces SET current_revision_sequence = ?, updated_at = ? WHERE id = ? AND owner_user_id = ? AND EXISTS (SELECT 1 FROM workspace_revisions WHERE id = ? AND workspace_id = ?)")
      .bind(revision.sequence, now, input.workspaceId, input.userId, revision.id, input.workspaceId),
  ];
  try {
    const results = await db.batch(statements);
    if ((results[1]?.meta?.changes ?? 0) === 0) throw new RevisionConflictError();
  } catch (error) {
...
export async function loadCurrentDesiredState(db: WorkspaceDatabase, userId: string, workspaceId: string): Promise<CloudRevision | { id: null; sequence: number; state: DesiredState; dispositionLedger: DispositionLedger }> {
  ...
  if (!row.id || row.manifestJson === null || row.lockfileJson === null) return { id: null, sequence: row.sequence, state: emptyDesiredState, dispositionLedger: emptyLedger };
  const state = validateCloudState({ manifest: JSON.parse(row.manifestJson), lockfile: JSON.parse(row.lockfileJson) } as CloudDesiredState);
  return { id: row.id, sequence: row.sequence, state, dispositionLedger: row.dispositionLedgerJson ? parseDispositionLedger(row.dispositionLedgerJson) : emptyLedger };
}

export const emptyDesiredState: DesiredState = { manifest: { version: 1, skills: [] }, lockfile: { version: 1, skills: [] } };
