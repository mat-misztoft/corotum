...
export async function handleWebMcpTool(
  ...
  const mutation = body.tool === "add_skill" || body.tool === "remove_skill" || body.tool === "update_skill" || body.tool === "set_skill_ref";
  ...
  if (mutation && !isSameOrigin(request)) return jsonError("Invalid request origin", 403);

  try {
    if (!mutation) {
      return Response.json(await executeWebMcpReadOnlyTool(db, { userId, hosted, tool: body.tool }));
    }
    if ((body.baseRevisionId !== null && typeof body.baseRevisionId !== "string") || typeof body.idempotencyKey !== "string") {
      return jsonError("A base revision and idempotency key are required for WebMCP mutations", 400);
    }
    return Response.json(await executeWebMcpMutationTool(db, {
      userId,
      hosted,
      tool: body.tool,
    ...
    if (error instanceof InvalidWebMcpToolError || error instanceof InvalidWebMcpMutationInputError) return jsonError(error.message, 400);
