...
async function gitRepo(
  ...
  return {
    directory,
    revision: (await git(["-C", directory, "rev-parse", "HEAD"])).trim(),
  };
}
...
describe("init adoption decisions", () => {
  test("locks known unchanged content to the resolved immutable revision, not HEAD", async () => {
    ...
    expect(
      outcome.kind === "source-backed" && outcome.source.revision,
    ).not.toBe("HEAD");
  ...
  test("modified Replace/Keep/Do-not-manage follow the documented per-skill outcomes", async () => {
    ...
    const before = await snapshot(
      join(skipHome, ".agents", "skills", "review", "SKILL.md"),
    );
    ...
    const replaced = await decideInitAdoptions({
      ...
      nonInteractive: false,
      prompt: interactive({
        chooseModified: async (names) => asModified(names, "replace"),
      }),
    });
    const kept = await decideInitAdoptions({
      ...
      nonInteractive: false,
      prompt: interactive({
        chooseModified: async (names) => asModified(names, "keep"),
      }),
    });
    const skipped = await decideInitAdoptions({
      ...
      nonInteractive: false,
      prompt: interactive({
        chooseModified: async (names) => asModified(names, "do-not-manage"),
      }),
    });
    ...
    expect(skipped[0]).toMatchObject({
      kind: "unmanaged",
      outcome: "DO_NOT_MANAGE",
      classification: "modified",
    });
    expect(
      await snapshot(join(skipHome, ".agents", "skills", "review", "SKILL.md")),
    ).toBe(before);
  });
  ...
  test("unknown artifact adoption scans first and invents no source", async () => {
    ...
    expect(
      outcome.kind === "artifact-backed" ? outcome.source : "missing",
    ).toBeUndefined();
  });
...
function interactive(
  ...
  return {
    ...
    chooseModified:
      overrides.chooseModified ??
      (async (names) => asModified(names, "do-not-manage")),
    chooseUnavailable: overrides.chooseUnavailable ?? (async () => []),
    chooseUnknown: overrides.chooseUnknown ?? (async () => []),
    chooseDuplicate:
      overrides.chooseDuplicate ?? (async () => "do-not-manage" as const),
  };
}
