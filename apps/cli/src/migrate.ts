...
export type MigrationResult =
  | Readonly<{ kind: "migrated"; revision: string; strategy: "replace" | "merge" }>
...
export class MigrationService {
  ...
  async migrate(strategy: MigrationStrategy): Promise<MigrationResult> {
    ...
    if ("conflicts" in state) return { kind: "conflict", skills: state.conflicts };

    const transition = migrationTransition(state.state);
    if (!transition)
      return { kind: "refused", reason: "Cannot migrate an empty desired state." };
    ...
    const result =
      destination.kind === "success"
        ...
        : typeof this.destination.bootstrap === "function"
          ...
          : { kind: "failure" as const, error: { code: "NETWORK_ERROR" as const, message: failureMessage(destination) } };
...
function emptyState(): DesiredState {
  return { manifest: { version: 1, skills: [] }, lockfile: { version: 1, skills: [] } };
}
...
function mergeDestination(destination: DesiredState, source: DesiredState):
  | Readonly<{ state: DesiredState }>
  | Readonly<{ conflicts: readonly string[] }> {
  const destinationById = new Map(destination.manifest.skills.map((skill) => [skill.id, skill]));
  const destinationByIdentity = new Map(destination.manifest.skills.map((skill) => [identity(skill), skill]));
  ...
  for (const skill of source.manifest.skills) {
    ...
    if ((sameId && !sameSkill(sameId, skill)) || (sameIdentity && !sameSkill(sameIdentity, skill))) {
  ...
  for (const skill of source.manifest.skills) {
    ...
    manifest.push(skill);
    const lock = source.lockfile.skills.find((candidate) => candidate.id === skill.id);
    if (lock) locks.push(lock);
  }
  return { state: validateDesiredState({ manifest: { version: 1, skills: manifest }, lockfile: { version: 1, skills: locks } }, "cloud") };
}
...
function failureMessage(result: Exclude<Awaited<ReturnType<StateProvider["pull"]>>, { kind: "success" }>): string {
  return result.kind === "failure" ? result.error.message : "Desired state is incomplete.";
}
