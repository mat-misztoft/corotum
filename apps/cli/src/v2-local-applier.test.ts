...
describe("V2LocalApplier", () => {
  test("materializes the exact locked source revision and persists verified local state", async () => {
    ...
    const revision = (
      await git(["-C", repository, "rev-parse", "HEAD"])
    ).trim();
    const contentHash = (await scanNormalizedContent(join(repository, "skill")))
      .contentHash;
    ...
    const desired: V2DesiredState = {
      manifest: { version: 2, skills: [{ id, name: "source", targets: [] }] },
      lockfile: {
        version: 2,
        skills: [
          {
    ...
    expect(
      await readFile(join(path, "canonical", "source", "SKILL.md"), "utf8"),
    ).toBe("# Pinned\n");
    expect(await local.state.load()).toMatchObject({
  ...
  test("materializes an artifact without source metadata and persists it", async () => {
    ...
    const storage = join(
      path,
      "storage",
      sourceKey("https://example.test/team/state.git"),
      locator,
    );
    await mkdir(dirname(storage), { recursive: true });
    await Bun.write(storage, archive.bytes);
    const desired: V2DesiredState = {
      manifest: { version: 2, skills: [{ id, name: "artifact", targets: [] }] },
      lockfile: {
        version: 2,
        skills: [
          {
            ...
            materialization: {
              kind: "artifact",
              artifact: { kind: "git-tree", locator, ...archive },
            },
          },
        ],
      },
    ...
    expect(
      await readFile(join(path, "canonical", "artifact", "SKILL.md"), "utf8"),
    ).toBe("# Local artifact\n");
    expect(await local.state.load()).toMatchObject({
      lastAppliedRevision: "revision-2",
      skills: {
        [id]: {
          name: "artifact",
  ...
  test("enabling an agent later exposes managed skills and disable removes only exposure", async () => {
    ...
    const desired: V2DesiredState = {
      manifest: {
        version: 2,
        skills: [{ id, name: "source", targets: "all" }],
      },
      lockfile: {
        version: 2,
        skills: [
    ...
    expect((await enabled.state.load())?.lastAppliedRevision).toBe(
      "revision-1",
    );
