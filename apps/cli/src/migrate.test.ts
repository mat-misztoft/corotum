      ...
      skills: [
        {
          ...
          skill: name,
          ref: "main",
          repository: `https://example.test/${name}.git`,
          revision: "abc123",
          path: name,
...
describe("MigrationService", () => {
  test("replaces Cloud destination without changing the Git source", async () => {
    ...
    const result = await new MigrationService(
      source.provider,
      target.provider as never,
    ).migrate("replace");
  ...
  test("merges independent skills while preserving their locked fields", async () => {
    ...
    expect(target.state().manifest.skills.map((skill) => skill.id)).toEqual([
      skillId("sk_b"),
      skillId("sk_a"),
    ]);
    expect(
      target
        .state()
        .lockfile.skills.map((skill) => [skill.revision, skill.contentHash]),
    ).toEqual([
      ["abc123", "sha256:destination"],
      ["abc123", "sha256:source"],
    ]);
  });
  ...
  test("leaves both providers unchanged for cancellation and same-skill conflicts", async () => {
    ...
    const target = provider({
      ...
      manifest: {
        version: 1,
        skills: [{ ...state("skill", "a").manifest.skills[0], ref: "release" }],
      },
    });
    ...
    expect(await service.migrate("merge")).toEqual({
      kind: "conflict",
      skills: ["skill"],
    });
    expect(target.pushes()).toBe(0);
  ...
  test("does not bootstrap a Git destination while PENDING_PUSH remains unresolved", async () => {
    ...
    const pendingDestination = {
      ...
      push: async () => ({
        kind: "failure" as const,
        error: { code: "CONFLICT" as const, message: "unexpected" },
      }),
      bootstrap: async () => {
        bootstrapped = true;
        return {
          kind: "success" as const,
          value: { revisionId: revisionId("new"), state: state("source", "a") },
        };
      },
    };
    const result = await new MigrationService(
