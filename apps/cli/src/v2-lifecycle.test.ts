...
function memoryProvider(
  ...
  ledger: DispositionLedger = emptyLedger,
): V2MutationProvider & {
  snapshot: () => {
    revisionId: string;
    state: V2DesiredState;
...
async function seedArtifact(path: string, name: string, body: string) {
  ...
  const desired: V2DesiredState = {
    manifest: {
      version: 2,
      skills: [{ id, name, targets: ["codex"], resolutionStatus: "RESOLVED" }],
    },
    lockfile: {
      version: 2,
      skills: [
...
function harness(path: string) {
  ...
  const applier = new V2LocalApplier(
    state,
    new CanonicalSkillStore(join(path, "canonical")),
    {
...
describe("v2 remove/unmanage/restore lifecycle", () => {
  test("remove writes REMOVE then deletes only hash-verified ownership", async () => {
    ...
    const { id, desired, applier, state, recovery, contentHash } =
      await install(path, "review");
    ...
    expect(await service.remove("review")).toMatchObject({
      kind: "success",
      operation: "REMOVE",
      revision: "rev-2",
    });
    expect(provider.snapshot().ledger.activeDispositions[id]).toMatchObject({
      disposition: "REMOVE",
      name: "review",
    });
    expect(provider.snapshot().state.manifest.skills).toEqual([]);
    expect(
      await lstat(join(path, "canonical", "review")).catch(() => null),
    ).toBeNull();
    expect(await lstat(target).catch(() => null)).toBeNull();
    expect(await recovery.load()).toBeNull();
    expect(await state.load()).toMatchObject({
  ...
  test("remove leaves an unmanaged same-name directory unchanged", async () => {
    const path = await root();
    const { id, desired, applier, state, recovery } = await install(
      path,
      "review",
    );
    const target = join(path, "home", ".codex", "skills", "review");
    ...
    expect(
      await readFile(join(path, "canonical", "review", "SKILL.md"), "utf8"),
    ).toBe("# Managed\n");
  ...
  test("unmanage writes UNMANAGE and converts a verified symlink to a copy", async () => {
    ...
    expect(await service.unmanage("review")).toMatchObject({
      kind: "success",
      operation: "UNMANAGE",
    });
    ...
    await state.save({
      ...
      skills: {
        [id]: {
          ...skill,
          targets: {
            [key]: { ...skill.targets[key]!, mode: "copy", expectedHash },
          },
        },
      },
    });
  ...
  test("restore repairs a missing recorded target from the exact lock and refuses drift", async () => {
    ...
    expect(await service.restore("review")).toMatchObject({
      kind: "success",
      operation: "RESTORE",
    });
    expect((await lstat(target)).isSymbolicLink()).toBe(true);
  ...
  test("restore never overwrites an unmanaged target", async () => {
    ...
    await state.save({
      ...saved!,
      skills: { [id]: { ...saved!.skills[id]!, targets: {} } },
    });
  ...
  test("remove of drifted canonical content is refused before desired-state mutation", async () => {
    ...
    await writeFile(
      join(path, "canonical", "review", "SKILL.md"),
      "# Drifted\n",
    );
    const provider = memoryProvider(desired);
    const service = new V2LifecycleService(provider, applier, state, recovery);
  ...
  test("desired-state success then local failure leaves a recovery marker and no synced revision", async () => {
    ...
    expect(provider.snapshot().ledger.activeDispositions[id]?.disposition).toBe(
      "REMOVE",
    );
    expect(await recovery.load()).toMatchObject({
      operation: "REMOVE",
  ...
  test("local success then operational-state failure is retried without a false synced status", async () => {
    const path = await root();
    const { id, desired, applier, recovery, state } = await install(
      path,
      "review",
    );
    const provider = memoryProvider(desired);
  ...
  test("retries a desired-persisted remove without pushing again", async () => {
    ...
    expect(
      await new V2LifecycleService(provider, failing, state, recovery).remove(
        id,
      ),
    ).toMatchObject({
      kind: "persisted-not-applied",
  ...
  test("re-add after unmanage with a modified copy is LOCAL_CONFLICT", async () => {
    ...
    const provider = memoryProvider(desired);
    const lifecycle = new V2LifecycleService(
      provider,
      applier,
      state,
      recovery,
    ...
    await expect(
      applier.apply({ state: next, revisionId: "rev-3", skillIds: [readded] }),
    ).rejects.toThrow(
      /not verified Corotum-owned content|Unmanaged or ambiguous target/,
    );
    expect(await readFile(join(target, "SKILL.md"), "utf8")).toBe(
      "# Edited after unmanage\n",
    );
    ...
    expect(
      planV2Reconcile(
        ...
        (entry) =>
          entry.classification === "LOCAL_CONFLICT" ||
          entry.classification === "DRIFTED",
      ),
    ).toBe(true);
  });
  ...
  test("materializes restore from a real Git archive lock", async () => {
    ...
    const revision = (
      await git(["-C", repositoryPath, "rev-parse", "HEAD"])
    ).trim();
    const contentHash = (
      await scanNormalizedContent(join(repositoryPath, "review"))
    ).contentHash;
    await writeFile(
      join(repositoryPath, "review", "SKILL.md"),
      "# Moved HEAD\n",
    );
