...
export class V2MutationService {
  ...
  async add(
    input: Readonly<{
      ...
      targets?: V2DesiredState["manifest"]["skills"][number]["targets"];
    }>,
  ): Promise<V2MutationResult> {
    const current = await this.provider.pull();
    const duplicate = current.state.manifest.skills.find(
      (skill) =>
        skill.name === input.name ||
        (skill.source?.repository === input.source.repository &&
          skill.source.path === input.source.path),
    );
    ...
    try {
      source = await this.resolver.resolve(input.source);
    } catch (error) {
    ...
    return this.persistAndApply(
      current,
      {
        ...
        lockfile: {
          version: 2,
          skills: [...current.state.lockfile.skills, lock],
        },
      },
      [id],
    );
  ...
  async adoptArtifact(
    input: Readonly<{
      ...
      contentHash: ArtifactMetadata["contentHash"];
      integrityHash: ArtifactMetadata["integrityHash"];
      sizeBytes: number;
      source?: SourceMetadata;
      targets: V2DesiredState["manifest"]["skills"][number]["targets"];
    }>,
    ...
    const current = await this.provider.pull();
    if (
      current.state.manifest.skills.some((skill) => skill.name === input.name)
    )
      return {
        kind: "refused",
        reason: "A managed skill already uses this name.",
      };
    const id = skillId(`sk_${crypto.randomUUID().replaceAll("-", "")}`);
  ...
  async setRef(nameOrId: string, ref: string): Promise<V2MutationResult> {
    ...
    if (!skill.source)
      return {
        ...
        skillId: skill.id,
        reason: "This artifact-backed skill has no source metadata.",
      };
    let source: SourceLock;
    try {
      source = await this.resolver.resolve({ ...skill.source, ref });
    } catch (error) {
      return refused(error);
    }
    return this.replaceSourceLock(
      current,
      skill.id,
      { ...skill.source, ref },
      source,
  ...
  async check(nameOrId?: string): Promise<
    readonly Readonly<{
      skillId: SkillId;
      name: string;
      status:
  ...
  > {
    ...
    const selected = nameOrId
      ? [select(current.state, nameOrId)].filter(Boolean)
      : current.state.manifest.skills;
    return Promise.all(
      selected.map(async (skill) => {
        ...
        const lock = current.state.lockfile.skills.find(
        ...
        try {
          ...
          return {
            ...
            status:
              ...
              lock.source?.revision === next.revision &&
              lock.source.contentHash === next.contentHash
                ? ("UP_TO_DATE" as const)
  ...
  async update(nameOrId?: string): Promise<readonly V2MutationResult[]> {
    ...
    for (const skill of selected) {
      ...
      try {
        ...
        const old = latest.state.lockfile.skills.find(
          (entry) => entry.id === skill!.id,
        );
        if (
          old?.materialization.kind === "source" &&
          old.source?.revision === source.revision &&
          old.source.contentHash === source.contentHash
        ) {
          results.push({
            kind: "success",
            skillId: skill!.id,
            revision: latest.revisionId,
          });
          continue;
        }
        const result = await this.replaceSourceLock(
          latest,
          skill!.id,
          skill!.source!,
          source,
        ...
        results.push(refused(error));
      }
    }
    return results;
  }

  private async replaceSourceLock(
    current: Awaited<ReturnType<V2MutationProvider["pull"]>>,
    id: SkillId,
    metadata: SourceMetadata,
    source: SourceLock,
  ): Promise<V2MutationResult> {
    const manifest = current.state.manifest.skills.find(
      (skill) => skill.id === id,
    );
    const existing = current.state.lockfile.skills.find(
      (lock) => lock.id === id,
    );
    if (!manifest)
      return {
    ...
    if (!existing && manifest.resolutionStatus !== "PENDING_RESOLUTION") {
      return {
        kind: "refused",
        reason: "Managed skill has no current materialization.",
      };
    }
    const lock: V2LockedSkill = {
      id,
      ...
      current,
      {
        manifest: {
          ...
          skills: current.state.manifest.skills.map((skill) =>
            skill.id === id
              ? { ...skill, source: metadata, resolutionStatus: "RESOLVED" }
              : skill,
          ),
        },
        lockfile: {
          version: 2,
          skills: existing
            ? current.state.lockfile.skills.map((entry) =>
                entry.id === id ? lock : entry,
              )
  ...
  private async persistAndApply(
    ...
    try {
      ...
      persisted = await this.provider.push({
        state: validateV2DesiredState(state),
...
function refused(error: unknown): V2MutationResult {
  return {
    kind: "refused",
    reason:
