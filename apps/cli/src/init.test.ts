...
const candidates: readonly InitCandidate[] = [
  ...
  },
  {
    agentId: "pi",
    name: "review",
    path: "/home/a/.pi/agent/skills/review",
    source: "owner/skills",
    contentHash: "sha256:exact",
  },
];
...
describe("CLI init adoption", () => {
  test("coalesces exact source-known copies into one managed skill with both targets", async () => {
    expect(coalesceInitCandidates(candidates)).toEqual([
      {
        source: "owner/skills",
        name: "review",
        contentHash: "sha256:exact",
    ...
    const result = await service.initialize({
      candidates,
      selected: coalesceInitCandidates(candidates),
    ...
    const input = calls[0] as {
      desired: { manifest: { skills: readonly { targets: unknown }[] } };
      plan: { operations: readonly { kind: string }[] };
      state: { skills: Record<string, { targets: Record<string, unknown> }> };
    };
    expect(input.desired.manifest.skills[0].targets).toEqual(["codex", "pi"]);
    expect(input.plan.operations).toHaveLength(1);
    expect(Object.keys(Object.values(input.state.skills)[0].targets)).toContain(
      "codex\0/home/a/.codex/skills/review",
    );
  ...
  test("requires an interactive choice for divergent copies", async () => {
    const divergent = [
      ...candidates,
      { ...candidates[1], contentHash: "sha256:other" },
    ];
  ...
  test("uses the explicit TTY canonical-copy choice for divergent same-name skills", async () => {
    ...
    const selected = await selectInitCandidates(
      divergent,
      false,
      async () => 1,
    );
  ...
  test("refuses divergent adoption without a TTY and leaves local state untouched", async () => {
    ...
    expect(await selectInitCandidates(divergent, true)).toEqual(
      coalesceInitCandidates(divergent),
    );
    const { calls, service } = fixture();
    expect(
      await service.initialize({
  ...
  test("bootstraps a genuinely empty Git desired-state repository before adoption", async () => {
    ...
    const bootstrap = new InitService(
      {
        pull: async () => ({
          ...
          error: {
            ...
            message: "Git repository has no HEAD.",
          },
        }),
        push: async () => {
          throw new Error("unreachable");
        ...
        bootstrap: async (state) => ({
          kind: "success" as const,
          value: { revisionId: revisionId("first"), state },
        }),
      },
      {
        resolve: async () => ({
    ...
    ).toMatchObject({ kind: "initialized", revision: "first" });
    expect(calls).toHaveLength(1);
  });
