...
const emptyState = (): V2DesiredState => ({
  manifest: { version: 2, skills: [] },
  lockfile: { version: 2, skills: [] },
});
const emptyLedger = (): DispositionLedger => ({
  version: 2,
  activeDispositions: {},
});
...
async function realGit(root: string) {
  ...
  await provider.push({
    state: emptyState(),
    ledger: emptyLedger(),
    baseRevision: await runGit("-C", worktree, "rev-parse", "HEAD"),
  });
  return provider;
}
...
function fakeCloud(
  ...
  let current = {
    state: initial?.state ?? emptyState(),
    ledger: initial?.ledger ?? emptyLedger(),
  };
  ...
  return {
    ...
    push: async (
      ...
      if (input.baseRevision !== revisionId)
        throw new Error("Cloud desired state has changed.");
...
test("Git-to-Cloud migration archives only artifact locks and retains the full ledger", async () => {
  ...
  try {
        ...
        skills: [
          {
            ...
            materialization: {
              ...
              artifact: {
                kind: "git-tree",
                locator: `artifacts/${artifactId}/${archive.integrityHash.slice(7)}`,
                ...archive,
              },
            },
          },
    ...
    let pushed:
      | Parameters<
          Parameters<typeof migrateV2GitToCloud>[0]["destination"]["push"]
        >[0]
      | undefined;
    ...
    expect(pushed).toMatchObject({
      baseRevision: "cloud-before",
      ledger,
      artifacts: { [artifactId]: archive.bytes },
    });
    const migrated = pushed!.state.lockfile.skills;
    expect(migrated.find((lock) => lock.id === sourceId)).toEqual(
      state.lockfile.skills[1],
    );
    expect(
      migrated.find((lock) => lock.id === artifactId)?.materialization,
    ).toEqual({
      kind: "artifact",
      artifact: {
        kind: "r2-tar-zst",
        ...
        locator: `workspaces/ws_1/artifacts/${artifactId}/${archive.integrityHash}.tar.zst`,
      },
    });
  } finally {
  ...
  try {
    ...
    const state: V2DesiredState = {
      manifest: {
        version: 2,
        skills: [
          { id, name: "adopted", targets: "all", resolutionStatus: "RESOLVED" },
        ],
      },
      lockfile: {
        version: 2,
    ...
    const revision = await migrateV2CloudToGit({
      ...
      destination: {
        readArtifact: async () => {
          throw new Error("Cloud artifact should not read Git storage");
        },
        pull: async () => ({
          revisionId: "git-before",
          state,
        ...
        push: async (input) => {
          ...
          expect(
            await readFile(join(input.artifacts[id], "SKILL.md"), "utf8"),
          ).toBe("# adopted\n");
          expect(input.state.lockfile.skills[0]?.materialization).toMatchObject(
            {
              kind: "artifact",
              artifact: { kind: "git-tree", contentHash: archive.contentHash },
            },
          );
...
test("v2 migration merge unions independent records and rejects identity or tombstone collisions", () => {
  ...
  expect(merged).toMatchObject({
    kind: "merged",
    state: { manifest: { skills: [{ id: destinationId }, { id: sourceId }] } },
  });
  expect(
    mergeV2MigrationSnapshots(source, {
      ...destination,
      state: {
        ...destination.state,
        manifest: {
          ...
          skills: [
            {
              ...destination.state.manifest.skills[0]!,
              id: skillId("sk_other"),
              name: "SOURCE",
            },
          ],
  ...
  ).toEqual({ kind: "conflict", skills: ["source"] });
  expect(
    mergeV2MigrationSnapshots(source, {
      ...destination,
      ledger: {
  ...
  const gitLock = {
    id: skillId("sk_gitkeep"),
    name: "gitkeep",
    materialization: {
      kind: "artifact" as const,
      artifact: {
        kind: "git-tree" as const,
        locator: "artifacts/gitkeep",
        contentHash: hash,
        integrityHash: hash,
        sizeBytes: 1,
      },
  ...
  const descriptors = mergeV2MigrationSnapshots(
    {
      state: {
        ...
        lockfile: { version: 2, skills: [gitLock] },
      },
      ledger: emptyLedger(),
    },
    {
      state: {
        ...
        lockfile: { version: 2, skills: [r2Lock] },
      },
      ledger: emptyLedger(),
    },
  );
...
test("Cloud-to-Git writes a fake-R2 archive to one real local-Git snapshot", async () => {
  ...
  try {
    ...
    await mkdir(artifactDirectory);
    await writeFile(
      join(artifactDirectory, "SKILL.md"),
      "# fake R2 artifact\n",
    );
    const archive = await createArtifactArchive(artifactDirectory);
    ...
    const cloudState: V2DesiredState = {
      manifest: {
        version: 2,
        skills: [
          { id, name: "fake-r2", targets: "all", resolutionStatus: "RESOLVED" },
        ],
      },
      lockfile: {
        version: 2,
    ...
    expect(after.state.lockfile.skills[0]?.materialization).toMatchObject({
      kind: "artifact",
      artifact: { kind: "git-tree", contentHash: archive.contentHash },
    });
    const restored = await provider.readArtifact(
      after.state.lockfile.skills[0]!,
    );
  ...
  try {
        ...
        skills: [
          {
            ...
            materialization: {
              ...
              artifact: {
                kind: "git-tree",
                locator: "artifacts/retained",
                ...archive,
                integrityHash: treeHash,
              },
            },
          },
        ],
    ...
    await migrateV2CloudToGit({
      ...
      destination: {
        ...
        push: async (input) => {
          expect(input.artifacts[id]).toBeDefined();
          return { revisionId: "git-after" };
        },
      },
    });
...
test("Git-to-Cloud uploads a real local-Git artifact to fake R2 and leaves source metadata in place", async () => {
  ...
  try {
      ...
      lockfile: {
        ...
        skills: [
          ...
          },
          {
            id: sourceId,
            name: "upstream",
            source: { ...source, revision: "a".repeat(40), contentHash: hash },
            materialization: { kind: "source", contentHash: hash },
          },
        ],
      },
    };
    const seeded = await provider.push({
      state,
      ledger,
      baseRevision: (await provider.pull()).revisionId,
    ...
    expect(
      cloud
        .snapshot()
        .state.lockfile.skills.find((lock) => lock.id === sourceId),
    ).toEqual(state.lockfile.skills[1]);
    const uploaded = cloud
      .snapshot()
      .state.lockfile.skills.find(
    ...
    if (uploaded?.kind !== "artifact")
      throw new Error("expected artifact lock");
    expect(uploaded.artifact.locator).toBe(
      `workspaces/ws_1/artifacts/${artifactId}/${uploaded.artifact.integrityHash}.tar.zst`,
    );
...
test("Git-to-Cloud leaves Git intact when Cloud transfer fails", async () => {
  ...
  try {
        ...
        skills: [
          {
            ...
            materialization: {
              ...
              artifact: {
                kind: "git-tree",
                locator: `artifacts/${id}/${integrityHash.slice(7)}`,
                contentHash,
                integrityHash,
                sizeBytes: 1,
              },
    ...
    const seeded = await provider.push({
      ...
      ledger: emptyLedger(),
      baseRevision: (await provider.pull()).revisionId,
      artifacts: { [id]: artifactDirectory },
    });
    await expect(
      migrateV2GitToCloud({
        source: seeded,
...
test("Git-to-Cloud-to-Git and Cloud-to-Git-to-Cloud preserve logical desired state", async () => {
  ...
  try {
      ...
      lockfile: {
        ...
        skills: [
          ...
          },
          {
            id: sourceId,
            name: "upstream",
            source: { ...source, revision: "b".repeat(40), contentHash: hash },
            materialization: { kind: "source", contentHash: hash },
          },
        ],
      },
    };
    const seeded = await gitA.push({
      state,
      ledger,
      baseRevision: (await gitA.pull()).revisionId,
      artifacts: { [artifactId]: artifactDirectory },
    });
    const cloud = fakeCloud();
    ...
    expect(
      backOnGit.state.lockfile.skills.find((lock) => lock.id === sourceId),
    ).toEqual(state.lockfile.skills[1]);
    expect(
      backOnGit.state.lockfile.skills.find((lock) => lock.id === artifactId)
        ?.materialization,
    ).toMatchObject({
      kind: "artifact",
    ...
    expect(cloudB.snapshot().state.manifest).toEqual(
      cloud.snapshot().state.manifest,
    );
    ...
    expect(
      cloudB
        .snapshot()
        .state.lockfile.skills.find((lock) => lock.id === artifactId)
        ?.materialization,
    ).toMatchObject({
      kind: "artifact",
      artifact: { kind: "r2-tar-zst", contentHash },
