...
export type InitStateProvider = StateProvider &
  Readonly<{
    push: (
      input: {
        state: DesiredState;
...
export type BootstrapInitStateProvider = InitStateProvider &
  Readonly<{
...
export function coalesceInitCandidates(
  ...
  return [...groups.values()].map((group) => ({
    ...
    targets: [
      ...new Set(group.map((candidate) => candidate.agentId)),
    ].sort() as AgentId[],
  }));
}

export function divergentCandidates(
  candidates: readonly InitCandidate[],
): readonly InitCandidate[] {
  const hashesByName = new Map<string, Set<string>>();
  for (const candidate of candidates) {
    hashesByName.set(
      candidate.name,
      new Set([
        ...(hashesByName.get(candidate.name) ?? []),
        candidate.contentHash,
      ]),
    );
  }
  return candidates.filter(
    (candidate) => (hashesByName.get(candidate.name)?.size ?? 0) > 1,
  );
}
...
export class InitService {
  ...
  async initialize(input: {
    ...
    const invalidSelection = validateSelections(
      input.candidates,
      input.selected,
    );
    ...
    if (input.nonInteractive && divergent.length > 0) {
      return {
        kind: "refused",
        reason:
          "Divergent local skills require an interactive canonical-copy selection.",
      };
    }
    if (
      !input.nonInteractive &&
      divergent.length > 0 &&
      !hasExplicitDivergentChoices(divergent, input.selected)
    ) {
    ...
    if (input.selected.length === 0) {
      return {
        kind: "refused",
        reason: "No source-resolved skills were selected for adoption.",
      };
    }
    ...
    if (
      ...
      current.value.state.manifest.skills.length > 0
    ) {
      return {
        kind: "refused",
      ...
      return {
        ...
        reason:
          current.kind === "failure"
            ...
            : "Initial desired state could not be loaded completely.",
      };
    }
    ...
    for (const [index, selected] of input.selected.entries()) {
      ...
      if (resolved.contentHash !== selected.contentHash) {
        return {
          kind: "refused",
          reason: `Repository content for ${selected.name} differs from the selected local copy.`,
        };
      }
      locks.push({
        id,
    ...
    const state: DesiredState = {
      manifest: {
        version: 1,
        skills: locks.map((lock, index) => ({
          id: lock.id,
          source: lock.source,
          skill: lock.skill,
          ref: lock.ref,
          targets: input.selected[index].targets,
    ...
    const pushed =
      current.kind === "success"
        ? await this.provider.push(
            { state, baseRevision: current.value.revisionId },
            transition,
          )
      ...
      return {
        ...
        reason:
          pushed.kind === "failure"
            ...
            : "Initial desired state could not be saved completely.",
      };
    }
        ...
        locks.map((lock, index) => [
          lock.id,
          {
            ...
            targets: Object.fromEntries(
              input.candidates
                .filter((candidate) =>
                  isSelectedCandidate(candidate, input.selected[index]),
                )
                .map((candidate) => [
                  `${candidate.agentId}\0${candidate.path}`,
                  {
    ...
    const execution = await this.executor.execute({
      ...
      plan: planReconcile(state, actual),
    });
    ...
    if (
      ...
      return {
        ...
        skillIds,
        reason:
          "Desired state was saved, but one or more local skill targets could not be adopted.",
      };
    }
...
function isSelectedCandidate(
  ...
  return (
    ...
    candidate.contentHash === selection.contentHash &&
    selection.targets.includes(candidate.agentId)
  );
}

function validateSelections(
  candidates: readonly InitCandidate[],
  selections: readonly InitSelection[],
): string | null {
  const identities = new Set<string>();
  for (const selection of selections) {
    const identity = `${selection.source}\0${selection.name}`;
    if (identities.has(identity))
      return `Skill ${selection.name} was selected more than once.`;
    identities.add(identity);
    if (
      selection.targets.length === 0 ||
      !candidates.some((candidate) => isSelectedCandidate(candidate, selection))
    ) {
...
function isBootstrapProvider(
  provider: InitStateProvider,
): provider is BootstrapInitStateProvider {
  return "bootstrap" in provider && typeof provider.bootstrap === "function";
}

function hasExplicitDivergentChoices(
  divergent: readonly InitCandidate[],
  selections: readonly InitSelection[],
): boolean {
  const names = new Set(divergent.map((candidate) => candidate.name));
  return [...names].every(
    (name) =>
      selections.filter((selection) => selection.name === name).length === 1,
  );
}
