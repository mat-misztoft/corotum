...
export function registerMigrateCommand(program: Command, io: CliIo): void {
  program
    ...
    .description("move desired state between ToolMirror Git Sync and Cloud")
    ...
    .action(async (destination: string, repository: string | undefined, options: { strategy?: MigrationStrategy; origin: string }) => {
      ...
      if (destination === "git" && !repository)
        throw new Error("Usage: toolmirror migrate git <repository>.");
      if (destination === "cloud" && repository)
        throw new Error("Usage: toolmirror migrate cloud.");
      ...
      try {
        ...
        if (!config.mode) throw new Error("Run toolmirror init before migrating.");
        if (config.mode === destination) throw new Error(`ToolMirror is already using ${destination === "cloud" ? "Cloud" : "Git Sync"}.`);
        ...
        const cloudProvider = async () => {
          ...
          if (!credentials.cloudDeviceToken || !config.workspaceId)
            throw new Error("Run toolmirror login before migrating to or from Cloud.");
        ...
        write(io, program, { outcome: "SUCCESS", status: "MIGRATED", revision: result.revision, strategy: result.strategy }, `Migrated desired state to ${destination === "cloud" ? "ToolMirror Cloud" : "Git Sync"}.\n`);
      } finally {
        await release();
      }
    });
