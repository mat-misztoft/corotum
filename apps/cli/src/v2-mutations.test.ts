...
import { type V2MutationProvider, V2MutationService } from "./v2-mutations";
...
): V2MutationProvider & { pushes: number } {
  let current = state;
  let pushes = 0;
  return {
    get pushes() {
    ...
    push: async (input) => {
      pushes++;
      current = input.state;
      return { revisionId: revision, state: current, ledger };
    },
  };
}
...
const resolver = {
  resolve: async (source: {
    ...
    path: string;
    ref: string;
  }) => ({ ...source, revision, contentHash: hash }),
};
...
describe("v2 mutation commands", () => {
  test("add locks an immutable source and does not alter an existing ref", async () => {
    ...
    expect(saved.state.lockfile.skills[0]?.materialization).toEqual({
      kind: "source",
      contentHash: hash,
    });
  });
  ...
  test("artifact adoption keeps optional provenance and null-source update is explicit", async () => {
    ...
    const adopted = await service.adoptArtifact({
      name: "local",
      targets: "all",
      artifactDirectory: "/staged/local",
      contentHash: hash,
      integrityHash: hash,
      sizeBytes: 1,
    });
  ...
  test("updates an adopted artifact through retained provenance while check remains read-only", async () => {
    ...
    const adopted = await service.adoptArtifact({
      ...
      source,
      targets: "all",
      artifactDirectory: "/staged/alpha",
      contentHash: hash,
      integrityHash: hash,
      sizeBytes: 1,
    });
  ...
  test("keeps a persisted snapshot recoverable when local application fails", async () => {
    ...
    expect(
      (await state.pull()).state.lockfile.skills[0]?.source?.revision,
    ).toBe(revision);
  });
  ...
  test("continues an update after a durable but locally failed application", async () => {
    ...
    const state: V2MutationProvider = {
      ...
      push: async (input) => {
        if (input.baseRevision !== `revision-${revisionNumber}`)
  ...
  test("set-ref resolves before persistence and check is read-only", async () => {
    ...
    expect(await service.check("alpha")).toEqual([
      {
        skillId: (await state.pull()).state.manifest.skills[0]?.id,
        name: "alpha",
        status: "UP_TO_DATE",
      },
    ]);
    ...
    expect((await state.pull()).state.manifest.skills[0]?.source?.ref).toBe(
      "v1",
    );
  });
});
