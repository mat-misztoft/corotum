...
const denylist: readonly Readonly<{
  pattern: string;
  ...
  {
    pattern: "*.secret",
    matches: (path) => basename(path).endsWith(".secret"),
  },
  ...
  {
    pattern: "credentials",
    matches: (path) => basename(path) === "credentials",
  },
  {
    pattern: "credentials.*",
    matches: (path) => basename(path).startsWith("credentials."),
  },
  { pattern: "secrets", matches: (path) => basename(path) === "secrets" },
  {
    pattern: "secrets.*",
    matches: (path) => basename(path).startsWith("secrets."),
  },
];
 ...
 */
export async function scanNormalizedContent(
  directory: string,
  ...
  for (const file of files) {
    if (file.path !== ignoreFileName && !ignored(file.path, ignoreRules))
      selected.push(file);
  }
...
async function enumerate(
  root: string,
  directory: string,
): Promise<NormalizedContentFile[]> {
  const directoryPath =
    directory === root ? "content root" : safeRelativePath(root, directory);
  const directoryStat = await lstat(directory).catch(() => null);
  if (!directoryStat?.isDirectory() || directoryStat.isSymbolicLink())
    throw unsafe(directoryPath, "changed or symbolic directory");
  ...
  for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
    ...
    if (stat.isSymbolicLink())
      throw unsafe(path, "symbolic link or reparse point");
    const denied = denylist.find(({ matches }) => matches(path));
    if (denied)
      throw new ContentScanError(
        "DENYLISTED_PATH",
        `Denied path ${path} matches ${denied.pattern}.`,
      );
    if (stat.isDirectory()) {
      ...
      if (!afterRead.isFile() || afterRead.isSymbolicLink())
        throw unsafe(path, "changed or symbolic entry");
...
async function readIgnoreFile(root: string): Promise<readonly IgnoreRule[]> {
  ...
  if (!stat.isFile() || stat.isSymbolicLink())
    throw unsafe(ignoreFileName, "ignore file is not a regular file");
  ...
  if (!afterRead.isFile() || afterRead.isSymbolicLink())
    throw unsafe(ignoreFileName, "changed or symbolic ignore file");
  if (source.trim().length === 0)
    throw new ContentScanError(
      "INVALID_IGNORE",
      ".corotumignore must not be empty.",
    );
  ...
  for (const rawLine of source.split(/\r?\n/)) {
    ...
    if (!validPattern(pattern))
      throw new ContentScanError(
        "INVALID_IGNORE",
        `Invalid .corotumignore pattern ${JSON.stringify(line)}.`,
      );
    if (seen.get(pattern) === !negated)
      throw new ContentScanError(
        "INVALID_IGNORE",
        `Conflicting .corotumignore pattern ${JSON.stringify(pattern)}.`,
      );
...
function ignored(path: string, rules: readonly IgnoreRule[]): boolean {
  ...
  for (const rule of rules)
    if (matchesIgnore(path, rule.pattern)) selected = rule.negated;
  return !selected;
}
...
function safeRelativePath(root: string, path: string): string {
  ...
  if (
    ...
    throw new ContentScanError(
      "INVALID_PATH",
      "Content entry has an unsafe relative path.",
    );
  }
  return value;
}
...
function unreadable(path: string): ContentScanError {
  return new ContentScanError("UNREADABLE_ENTRY", `Unreadable path ${path}.`);
}
function unsafe(path: string, reason: string): ContentScanError {
  return new ContentScanError(
    "UNSAFE_ENTRY",
