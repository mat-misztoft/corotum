...
import { ContentScanError, scanNormalizedContent } from "./normalized-content";
...
describe("normalized content scanning", () => {
  test("applies ordered ignore rules deterministically and excludes its configuration", async () => {
    ...
    expect(first.files.map((file) => file.path)).toEqual(["a.md", "keep.tmp", "z.md"]);
    expect(second).toEqual(first);
  });

  test("ignored content does not affect the normalized hash", async () => {
    const root = await skill({ ".corotumignore": "ignored.txt\n", "SKILL.md": "ok", "ignored.txt": "one" });
    const before = await scanNormalizedContent(root);
    await writeFile(join(root, "ignored.txt"), "two");
    expect((await scanNormalizedContent(root)).contentHash).toBe(before.contentHash);
  });

  test.each([
    [".env", ".env"], [".env.local", ".env.*"], [".npmrc", ".npmrc"], [".netrc", ".netrc"],
    ["certificate.pem", "*.pem"], ["private.key", "*.key"], ["archive.p12", "*.p12"], ["archive.pfx", "*.pfx"],
    ["value.secret", "*.secret"], ["id_rsa", "id_rsa"], ["id_dsa", "id_dsa"], ["id_ecdsa", "id_ecdsa"],
    ["id_ed25519", "id_ed25519"], ["credentials", "credentials"], ["credentials.json", "credentials.*"],
    ["secrets", "secrets"], ["secrets.json", "secrets.*"],
  ])("rejects denylisted %s before returning content", async (path, rule) => {
    const root = await skill({ ".corotumignore": `${path}\n`, [path]: "secret" });
    await expect(scanNormalizedContent(root)).rejects.toMatchObject<Partial<ContentScanError>>({ code: "DENYLISTED_PATH", message: expect.stringContaining(rule) });
  });
  ...
  test("rejects symlinks and malformed, empty, or conflicting ignore files without following them", async () => {
    ...
    await expect(scanNormalizedContent(root)).rejects.toMatchObject({ code: "UNSAFE_ENTRY" });

    for (const ignore of ["", "../escape", "*.md\n!*.md\n"]) {
      const caseRoot = await skill({ ".corotumignore": ignore, "SKILL.md": "ok" });
      await expect(scanNormalizedContent(caseRoot)).rejects.toMatchObject({ code: "INVALID_IGNORE" });
    }
  });
  ...
  test("rejects denylisted directories even when ignored", async () => {
    const root = await skill({ ".corotumignore": ".env/**\n", ".env/token": "secret" });
    await expect(scanNormalizedContent(root)).rejects.toMatchObject<Partial<ContentScanError>>({
  ...
  test("rejects an unreadable file without returning a partial result", async () => {
    const root = await skill({ "SKILL.md": "ok", "private.txt": "not readable" });
    const privatePath = join(root, "private.txt");
    await chmod(privatePath, 0o000);
    try {
      await expect(scanNormalizedContent(root)).rejects.toMatchObject({ code: "UNREADABLE_ENTRY" });
    } finally {
  ...
  test("rejects a symlinked content root", async () => {
    ...
    await expect(scanNormalizedContent(linkedRoot)).rejects.toMatchObject({ code: "INVALID_PATH" });
  });
});
