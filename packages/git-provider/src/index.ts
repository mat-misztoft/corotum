...
} from "../../skills-adapter/src/artifact-archive";
import {
...
export class GitStateProvider implements StateProvider {
    ...
    try {
      ...
      if (await this.hasHead(cache)) {
        return {
          ...
          error: {
            code: "CONFLICT",
            message: "Corotum is already initialized for this Git repository.",
          },
        };
      }
      ...
      await Promise.all([
        writeFile(
          join(cache, manifestFile),
          serializeManifest(validated.manifest),
        ),
        writeFile(
          join(cache, lockfileFile),
          serializeLockfile(validated.lockfile),
        ),
      ]);
      ...
      try {
        ...
        return {
          kind: "failure",
          error: { code: "CONFLICT", message: pendingPushMessage(error) },
        };
      }
  ...
  private async hasHead(cache: string): Promise<boolean> {
    ...
    throw new Error(
      result.stderr.trim() ||
        "Git could not inspect the desired-state repository.",
    );
  }
...
export class V2ArtifactConsentRequiredError extends Error {
  constructor() {
    super(
      "Git artifact publishing requires explicit consent before local content is committed.",
    );
...
export class V2GitStateProvider {
  ...
  async readArtifact(lock: V2LockedSkill): Promise<ArtifactArchive> {
    ...
    const current = snapshot.state.lockfile.skills.find(
      (skill) => skill.id === lock.id,
    );
    if (
      current?.materialization.kind !== "artifact" ||
      current.materialization.artifact.integrityHash !==
        lock.materialization.artifact.integrityHash ||
      current.materialization.artifact.contentHash !==
        lock.materialization.artifact.contentHash
    ) {
      throw new Error(
        `Artifact ${lock.id} does not match the verified Git state.`,
      );
    }
    const archive = await createArtifactArchive(
      join(await this.cache(), this.artifactLocator(current)),
    );
  ...
  async push(
    ...
    if (changedArtifacts.length > 0) {
      if (!this.confirmArtifactWrite)
        throw new V2ArtifactConsentRequiredError();
    ...
    try {
      ...
      await writeFile(
        join(staging, v2ManifestFile),
        serializeV2Manifest(state.manifest),
      );
      await writeFile(
        join(staging, v2LockfileFile),
        serializeV2Lockfile(state.lockfile),
      );
      await writeFile(
        join(staging, v2TransitionsFile),
        serializeDispositionLedger(input.ledger),
      );
      ...
      await rename(
        join(staging, v2TransitionsFile),
        join(cache, v2TransitionsFile),
      );
      await rm(join(cache, artifactsDirectory), {
        force: true,
        recursive: true,
      });
      if (await exists(join(staging, artifactsDirectory)))
        await rename(
          join(staging, artifactsDirectory),
          join(cache, artifactsDirectory),
        );
      await this.command(cache, [
        "add",
        "--",
      ...
      const trackedArtifacts = await this.runGit({
        args: ["ls-files", "--error-unmatch", artifactsDirectory],
        cwd: cache,
      });
      if (trackedArtifacts.exitCode === 0)
      ...
      const changed = await this.runGit({
        args: ["diff", "--cached", "--quiet"],
        cwd: cache,
      });
      if (changed.exitCode === 1) {
        await this.command(cache, [
          ...
          "--no-gpg-sign",
          "-m",
          "corotum: persist v2 desired state",
        ]);
        committed = true;
        ...
        throw new Error(
          changed.stderr || "Git could not inspect staged state.",
        );
  ...
  private async changedArtifacts(
    ...
    return next.lockfile.skills
      .filter(
        (skill) =>
          skill.materialization.kind === "artifact" &&
          priorById.get(skill.id) !==
            skill.materialization.artifact.integrityHash,
      )
      .map((skill) => skill.id);
  }
  ...
  private async stageArtifacts(
    ...
    for (const lock of state.lockfile.skills) {
      if (lock.materialization.kind === "source") {
        if (supplied[lock.id])
          throw new Error(
            `Source-backed skill ${lock.id} must not have an artifact.`,
          );
        continue;
      }
      ...
      const source =
        supplied[lock.id] ?? ((await exists(cached)) ? cached : undefined);
      if (!source)
        throw new Error(
          `Artifact-backed skill ${lock.id} is missing its artifact tree.`,
        );
      const destination = join(staging, locator);
      const scanned = await scanNormalizedContent(source);
      if (scanned.contentHash !== lock.materialization.artifact.contentHash)
        throw new Error(`Artifact ${lock.id} content hash does not match.`);
      ...
      if (
        (await gitTreeHash(destination)) !==
        lock.materialization.artifact.integrityHash
      )
        throw new Error(`Artifact ${lock.id} tree integrity does not match.`);
    }
    for (const id of Object.keys(supplied))
      if (!expected.has(id))
        throw new Error(`Artifact supplied for non-artifact skill ${id}.`);
  }

  private async verifyStaged(
    staging: string,
    expected: V2DesiredState,
  ): Promise<void> {
    const manifest = parseV2Manifest(
      await readFile(join(staging, v2ManifestFile), "utf8"),
    );
    const lockfile = parseV2Lockfile(
      await readFile(join(staging, v2LockfileFile), "utf8"),
      manifest,
    );
  ...
  private artifactLocator(
    lock: V2DesiredState["lockfile"]["skills"][number],
  ): string {
    if (lock.materialization.kind !== "artifact")
      throw new Error(`Skill ${lock.id} has no artifact.`);
    const locator = `${artifactsDirectory}/${lock.id}/${lock.materialization.artifact.integrityHash.slice("sha256:".length)}`;
    if (lock.materialization.artifact.locator !== locator)
      throw new Error(`Artifact ${lock.id} has an invalid Git locator.`);
    return locator;
  }

  private async verifyArtifactTrees(
    root: string,
    state: V2DesiredState,
  ): Promise<void> {
    for (const lock of state.lockfile.skills)
      if (lock.materialization.kind === "artifact") {
        ...
        if (
          scanned.contentHash !== lock.materialization.artifact.contentHash ||
          (await gitTreeHash(path)) !==
            lock.materialization.artifact.integrityHash
        )
          throw new Error(`Artifact ${lock.id} readback verification failed.`);
      }
  }
  ...
  private v2PendingPath(): string {
    return join(
      this.storagePath,
      `${sourceKey(normalizeGitSource(this.source))}${v2PendingPushSuffix}`,
    );
  }
  ...
  private async readV2Pending(): Promise<V2PendingPush | null> {
    try {
      return JSON.parse(
        await readFile(this.v2PendingPath(), "utf8"),
      ) as V2PendingPush;
    } catch (error) {
      if (
        typeof error === "object" &&
  ...
  private async clearV2Pending(): Promise<void> {
    await rm(this.v2PendingPath(), { force: true });
  }
  ...
  private async retryPendingPush(cache: string): Promise<V2PendingPushStatus> {
    ...
    const [base, remote, local] = await Promise.all([
      this.readAt(cache, pending.baseRevision),
      this.readAt(cache, upstream),
      this.readAt(cache, head),
    ]);
    ...
    await this.command(cache, [
      ...
      "--no-gpg-sign",
      "-m",
      "corotum: replay v2 pending state",
    ]);
    try {
      await this.command(cache, ["push", "origin", "HEAD"]);
      await this.clearV2Pending();
  ...
  private async readAt(
    cache: string,
    revision: string,
  ): Promise<Omit<V2GitStateEnvelope, "revisionId">> {
    const manifest = parseV2Manifest(
      await this.show(cache, revision, v2ManifestFile),
    );
    const lockfile = parseV2Lockfile(
      await this.show(cache, revision, v2LockfileFile),
      manifest,
    );
    return {
      state: validateV2DesiredState({ manifest, lockfile }),
      ledger: parseDispositionLedger(
        await this.show(cache, revision, v2TransitionsFile),
      ),
    };
  }
  ...
  private async writeSnapshot(
    ...
    await Promise.all([
      writeFile(
        join(cache, v2ManifestFile),
        serializeV2Manifest(state.manifest),
      ),
      writeFile(
        join(cache, v2LockfileFile),
        serializeV2Lockfile(state.lockfile),
      ),
      writeFile(
        join(cache, v2TransitionsFile),
        serializeDispositionLedger(ledger),
      ),
    ]);
  }

  private async restoreMergedArtifacts(
    cache: string,
    ...
    for (const lock of state.lockfile.skills)
      if (lock.materialization.kind === "artifact") {
        const locator = this.artifactLocator(lock);
        const source = (await this.gitPathExists(
          cache,
          remoteRevision,
          locator,
        ))
          ? remoteRevision
          : localRevision;
        if (!(await this.gitPathExists(cache, source, locator)))
          throw new Error(
            `Artifact ${lock.id} is missing from both pending snapshots.`,
          );
  ...
  private async gitPathExists(
    ...
    const result = await this.runGit({
      args: ["cat-file", "-e", `${revision}:${path}`],
      cwd: cache,
    });
    if (result.exitCode === 0) return true;
    if (result.exitCode === 1 || result.exitCode === 128) return false;
    throw new Error(
      result.stderr.trim() || "Git could not inspect an artifact.",
    );
  }
  ...
  private async isAncestor(
    ...
    if (result.exitCode === 0) return true;
    if (result.exitCode === 1) return false;
    throw new Error(result.stderr || "Git could not compare revisions.");
  }

  private async read(cache: string): Promise<V2GitStateEnvelope> {
    const revision = await this.revision(cache);
    const manifest = parseV2Manifest(
      await readFile(join(cache, v2ManifestFile), "utf8"),
    );
    const lockfile = parseV2Lockfile(
      await readFile(join(cache, v2LockfileFile), "utf8"),
      manifest,
    );
    ...
    return {
      ...
      state,
      ledger: parseDispositionLedger(
        await readFile(join(cache, v2TransitionsFile), "utf8"),
      ),
    };
  }
  ...
  private async output(cwd: string, args: readonly string[]): Promise<string> {
    return new TextDecoder()
      .decode((await this.command(cwd, args)).stdout)
      .trim();
  }
  private async command(
    cwd: string | undefined,
...
function mergeV2Snapshots(
  base: V2Snapshot,
  remote: V2Snapshot,
  local: V2Snapshot,
): V2SnapshotMerge {
  const entries = (state: V2DesiredState) =>
    new Map(
      state.manifest.skills.map((skill) => [
        skill.id,
        {
          manifest: skill,
          lock: state.lockfile.skills.find((lock) => lock.id === skill.id),
        },
      ]),
    );
  const before = entries(base.state);
  const theirs = entries(remote.state);
  const ours = entries(local.state);
  const ids = new Set([...before.keys(), ...theirs.keys(), ...ours.keys()]);
  const merged: {
    manifest: V2DesiredState["manifest"]["skills"][number];
    lock?: V2DesiredState["lockfile"]["skills"][number];
  }[] = [];
  const conflicts: string[] = [];
  for (const id of ids) {
    const baseEntry = before.get(id);
    const remoteEntry = theirs.get(id);
    const localEntry = ours.get(id);
    const same = (left: unknown, right: unknown) =>
      JSON.stringify(left) === JSON.stringify(right);
    const remoteChanged = !same(baseEntry, remoteEntry);
    const localChanged = !same(baseEntry, localEntry);
    if (remoteChanged && localChanged && !same(remoteEntry, localEntry)) {
      conflicts.push(id);
      continue;
    }
  ...
  const ledger: Record<
    keyof DispositionLedger["activeDispositions"],
    DispositionEntry
  > = {
    ...remote.ledger.activeDispositions,
  };
  for (const [id, entry] of Object.entries(local.ledger.activeDispositions) as [
    keyof DispositionLedger["activeDispositions"],
    DispositionEntry,
  ][]) {
    const remoteEntry = remote.ledger.activeDispositions[id];
    if (remoteEntry && remoteEntry.disposition !== entry.disposition) {
      conflicts.push(id);
      continue;
    }
    ledger[id] =
      !remoteEntry || entry.effectiveSequence >= remoteEntry.effectiveSequence
        ? entry
        : remoteEntry;
  }
  if (conflicts.length > 0)
    return { kind: "conflict", skillIds: [...new Set(conflicts)].sort() };
  ...
  return {
    ...
    state: validateV2DesiredState({
      manifest: { version: 2, skills: merged.map((entry) => entry.manifest) },
      lockfile: {
        version: 2,
        skills: merged.flatMap((entry) => (entry.lock ? [entry.lock] : [])),
      },
    }),
    ledger: parseDispositionLedger(
      JSON.stringify({ version: 2, activeDispositions: ledger }),
    ),
  };
}

/** Deterministic integrity hash for a sanitized plain Git artifact tree. */
export async function gitTreeHash(
  directory: string,
): Promise<`sha256:${string}`> {
  const scanned = await scanNormalizedContent(directory);
  const hasher = new Bun.CryptoHasher("sha256");
  hasher.update("corotum-git-tree-v1\\0");
  for (const file of scanned.files) {
